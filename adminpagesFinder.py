...
def admin_finder():
	...
	root.geometry('1500, 1000')
	# set the image of the window
	bckgrnd_img = Image.open('dark.png')
...
'adm/index.brf','adm.brf','affiliate.brf','adm_auth.brf','memberadmin.brf','administratorlogin.brf','cpanel','cpanel.php','cpanel.html',)
	...
	def check_directory(directory):
		full_url = url_entry.get() + directory
		try:
	...
	def start_program():
		...
		check_directories(url_entry.get(), directories)
	...
	def upload_file():
		...
		with open(file_path, 'r') as file:
			...
			# Get the URL from the entry
		url = url_entry.get()
		# call the check_directories function
		check_directories(url, directories)
